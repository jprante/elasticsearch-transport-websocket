...
public class NettyHttpChannel extends HttpChannel {
    ...
    private final NettyWebSocketServerTransport transport;
    ...
    public void sendResponse(RestResponse response) {
        ...
        org.elasticsearch.common.bytes.BytesReference content = response.content();
        boolean addedReleaseListener = false;
        try {
            buffer = response.contentThreadSafe() ?
                    ChannelBuffers.wrappedBuffer(content.toBytes(), 0, content.length()) :
                    ChannelBuffers.copiedBuffer(content.toBytes(), 0, content.length());
